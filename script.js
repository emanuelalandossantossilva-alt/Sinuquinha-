...
// Funções das Bolas
const bolas = [];
function registrarBolaNoPlacar(num, cor, tipo) {
    if(num === 0 || num === 1) return;
    const container = document.getElementById(tipo === 'impar' ? 'placar-lisas' : 'placar-listras');
    const icon = document.createElement('div');
    icon.className = 'ball-icon';
    icon.style.background = cor;
    if(tipo === 'par') icon.style.border = '2px solid white';
    icon.innerText = num;
...
function criarBola(cor, num, pos) {
    ...
    group.userData = { vx:0, vz:0, ativa:true, caindo: false, isBranca: num===".", numero: num === "." ? 0 : parseInt(num), tipo: (parseInt(num)||0)%2===0?'par':'impar', mesh: b, glow: glow, corReal: cor };
...
function removerBolaAdversario(qtd) {
    ...
    for(let i=0; i < qtd && i < oponenteBolas.length; i++) { 
        registrarBolaNoPlacar(oponenteBolas[i].userData.numero, oponenteBolas[i].userData.corReal, oponenteBolas[i].userData.tipo);
        oponenteBolas[i].userData.ativa = false; scene.remove(oponenteBolas[i]); 
    }
}
        ...
        bolas.forEach(b => {
            ...
            if(b.userData.caindo) {
                ...
                if(b.position.y < ALTURA_MESA-2) { 
                    ...
                    if(!eraBranca) registrarBolaNoPlacar(b.userData.numero, b.userData.corReal, b.userData.tipo);
